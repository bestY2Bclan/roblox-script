...
Misc Tools, Vertex UI removed, UI password gate.
Password: Y2B
===============================================================]]
...
PassFrame.Size = UDim2.new(0, 260, 0, 130)
PassFrame.Position = UDim2.new(0.5, -130, 0.5, -65)
...
PassSub.Size = UDim2.new(1, -20, 0, 18)
...
PassStatus.Size = UDim2.new(1, -20, 0, 18)
PassStatus.Position = UDim2.new(0, 10, 1, -24)
PassStatus.BackgroundTransparency = 1
PassStatus.Font = Enum.Font.Gotham
PassStatus.TextSize = 12
PassStatus.TextColor3 = Color3.fromRGB(255, 120, 120)
PassStatus.Text = ""
...
local function checkPassword()
    ...
    if input == EXPECTED_PASSWORD then
        ...
        PassStatus.Text = "Invalid password."
        TweenService:Create(
            PassBtn,
            TweenInfo.new(0.15),
...
task.wait(1.5)
local fadeTween = TweenService:Create(BG, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 1})
fadeTween:Play()
...
local function createToggle(parent, text, default, callback)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(
            btn,
            TweenInfo.new(0.15),
            {BackgroundColor3 = state and Color3.fromRGB(200, 60, 60) or Color3.fromRGB(80, 0, 0)}
        ):Play()
...
-- Tabs: Player, Combat, ESP, Misc
local PlayerBtn, PlayerGlow = createTabButton("🏃 Player", 1, Color3.fromRGB(220, 80, 80))
local CombatBtn, CombatGlow = createTabButton("🗡️ Combat", 2, Color3.fromRGB(255, 110, 110))
local ESPBtn, ESPGlow       = createTabButton("👁️ ESP", 3, Color3.fromRGB(255, 150, 150))
local MiscBtn, MiscGlow     = createTabButton("⚙️ Misc", 4, Color3.fromRGB(255, 200, 200))
...
local function applyStatsToChar(char)
    ...
    local function setJump(h, enabled, value)
        if h:FindFirstChild("UseJumpPower") ~= nil then
            h.UseJumpPower = false
        end

        if h.JumpHeight then
            h.JumpHeight = enabled and (value / 10) or 7.2
        else
            if h:FindFirstChild("UseJumpPower") ~= nil then
                h.UseJumpPower = true
            end
            h.JumpPower = enabled and value or 50
        end
    end

    setJump(hum, jumpEnabled, jumpValue)
end
...
local function doAutoHeal()
    ...
    if hum.Health > 0 and lastSafeCFrame then
...
task.spawn(function()
    while task.wait(0.2) do
        ...
        if hum.Health > 0 and hum.Health <= healHPThreshold and isRagdolled(char) then
            doAutoHeal()
        end
    end
end)
...
-------------------------------------------------------
-- COMBAT MODES + DRAG COMBO (FOLLOW => AUTO DRAG)
-------------------------------------------------------
...
-- Auto-drag combo tied into follow toggle
...
local followToggle = createToggle(CombatPage, "Follow Kill Target (noclip + orbit)", followTarget, function(v)
    ...
    -- Tie drag combo to follow target
...
-- Continuous TP front while autoDragCombo on (used by follow as well)
task.spawn(function()
    while true do
        if autoDragCombo and selectedTarget then
            ...
            if target and target.Character then
                ...
                if tHRP then
                    ...
                    local hrp = char:FindFirstChild("HumanoidRootPart")
                    if hrp then
                        hrp.CFrame = tHRP.CFrame * CFrame.new(0, upOff, frontOff)
                    end
                end
            end
        end
...
-- Manual toggle kept for UI (still works, but follow will also control it)
    ...
    if followTarget and selectedTarget and hrp then
        ...
        if targetPlayer and targetPlayer.Character then
            ...
            if targetHRP then
                ...
                local groundY = math.max(targetHRP.Position.Y - 3, hrp.Position.Y - 10)
                local desiredY = math.max(groundY, hrp.Position.Y - 2)
                ...
                if moveDist > 0.5 then
                    ...
                    hrp.CFrame = hrp.CFrame + moveDir.Unit * step
                end
            end
        end
    end
